# 🐯 Week 09 · Data Preparation & Modeling

### _Growth Marketing Bootcamp Portfolio_

> **Weekly Goal:** 전처리된 데이터를 활용해 모델링 과정을 설계합니다.

🏠 [Main Portfolio](../README.md)

---

### 🎯 Focus

- 데이터 전처리
- 특성 이해
- 모델링 기초

---

### 🛠 Skills & Tools

| What I Practice | Tools |
|---|---|
| 데이터 전처리<br>특성 이해<br>모델링 기초 | Python<br>Pandas<br>Scikit-learn |

---

### 📓 TIL

| No. | 날짜 | 주제 | 상태 |
|---:|---|---|---|
| 01 | 2026-09-14 | [데이터 수집과 전처리](./2026-09-14_daily_til.ipynb) | 작성 예정 |
| 02 | 2026-09-15 | [데이터 수집과 전처리](./2026-09-15_daily_til.ipynb) | 작성 예정 |
| 03 | 2026-09-16 | [데이터 수집과 전처리](./2026-09-16_daily_til.ipynb) | 작성 예정 |
| 04 | 2026-09-17 | [머신러닝](./2026-09-17_daily_til.ipynb) | 작성 예정 |
| 05 | 2026-09-18 | [머신러닝](./2026-09-18_daily_til.ipynb) | 작성 예정 |

> 노트북과 같은 이름의 HTML도 미리 준비되어 있습니다. 해당 날짜의 파일을 열어 작성하면 됩니다.

### 🔍 Recruiter Takeaway

> 분석 목적에 맞는 입력 데이터를 만들고 모델링 흐름을 구성하는 역량

---

[← Week 08](../week08/) · [Main Portfolio](../README.md) · [Week 10 →](../week10/)
